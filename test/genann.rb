assert 'example1' do
  ...
  puts sprintf('Output for [%1.f, %1.f] is %1.f.\n', input[0][0], input[0][1], ann.run(input_buf[0]).unpack('E')[0])
  puts sprintf('Output for [%1.f, %1.f] is %1.f.\n', input[1][0], input[1][1], ann.run(input_buf[1]).unpack('E')[0])
  puts sprintf('Output for [%1.f, %1.f] is %1.f.\n', input[2][0], input[2][1], ann.run(input_buf[2]).unpack('E')[0])
  puts sprintf('Output for [%1.f, %1.f] is %1.f.\n', input[3][0], input[3][1], ann.run(input_buf[3]).unpack('E')[0])
end